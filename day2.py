...
Although it hasn't change
"""

instructions = """
...
def is_valid(pos):
  return (pos[0] >= 0 and pos[0] <= 2 and pos[1] >= 0 and pos[1] <= 2)
...
def get_bathroom_code(instructions):
  ...
  for line in instructions.split('\n'):
    ...
    for direction in line:
      if direction not in ["U", "D", "L", "R"]: continue
...
def is_valid_two(pos):
  if pos[0] < 0 or pos[1] < 0: return False
  try:
    value = keypad_two[pos[0]][pos[1]]
    return True if value != 0 else False
  except:
    return False

def get_bathroom_code_two(instructions):
  pos = (2, 0) #5
  code = ""
  for line in instructions.split('\n'):
    if not line: continue
    for direction in line:
      if direction not in ["U", "D", "L", "R"]: continue
      new_pos = get_pos(pos, direction)
      if is_valid_two(new_pos):
        pos = new_pos
    code += str(keypad_two[pos[0]][pos[1]])

  return code

# Part 1 Answer
print (get_bathroom_code(instructions))
...
# Part 2 Answer
print (get_bathroom_code_two(instructions))
